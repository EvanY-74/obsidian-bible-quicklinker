...
const DEFAULT_SETTINGS: Partial<PluginSettings> = {
    ...
    sameChapterMultiVerseFormat: "[[{book} {chapter}:{verse}]]-[[{verse}]]",
    diffChapterMultiVerseFormat: "[[{book} {chapter}:{verse}]]-[[{chapter}:{verse}]]",
...
export default class BibleReferencePlugin extends Plugin {
    ...
    async onload() {
        ...
        this.registerEditorSuggest(new QuickSuggest(this, this.settings));
...
class QuickSuggest extends EditorSuggest<string> {
    ...
    constructor(plugin: BibleReferencePlugin, settings: PluginSettings) {
        super(plugin.app);
        this.plugin = plugin;
    ...
    getSuggestions(context: EditorSuggestContext): string[] {
        try {
            ...
            return [
                ...
                ...(reference.verse !== undefined ? ["Paste verse" + multipleVerses] : [])
            ];
        } catch (err) {
    ...
    async selectSuggestion(suggestion: string): Promise<void> {
        ...
        try {
            const reference = JSON.parse(this.context.query) as MultiReference;
            let replacement = '';
            ...
            if (suggestion.startsWith("Link")) {
                ...
                if (reference.endVerse === undefined) {
                    const firstVerse = await getVerse(reference.book, reference.chapter, reference.verse, this.settings.verseHeadingLevel);
                    if (firstVerse instanceof Error) {
                        console.log(firstVerse.message);
                        new Notice(firstVerse.message, 5000);
                        return;
                    } else replacement = firstVerse;
                } else {
                    let { chapter, verse } = reference;
                    do {
                        const fetchedVerse = await getVerse(reference.book, chapter, verse, this.settings.verseHeadingLevel);
                        if (fetchedVerse instanceof Error) {
                            console.log(fetchedVerse.message);
                            new Notice(fetchedVerse.message, 5000);
                            return;
                        } else replacement += `${verse} ${fetchedVerse} `;
                        if (verse < books[reference.book][chapter]) verse++;
                        else {
                            chapter++;
                            verse = 1;
...
async function getVerse(book: string, chapter: number, verse: number, verseHeadingLevel: number | undefined): Promise<string | Error> {
    let file;
    const files = this.app.vault.getFiles();
    for (const currentFile of files) {
        if (currentFile.name === `${book} ${chapter}.md`) file = currentFile;
    }
...
function generateLink(reference: Reference | MultiReference, settings: PluginSettings): string {
    ...
    else if (chapter == endChapter)  return `[[${book} ${chapter}#${verse}|${book} ${chapter}:${verse}-]][[${book} ${endChapter}#${endVerse}|${endVerse}]]`;
    else 							 return `[[${book} ${chapter}#${verse}|${book} ${chapter}:${verse}-]][[${book} ${endChapter}#${endVerse}|${endChapter}:${endVerse}]]`;
}